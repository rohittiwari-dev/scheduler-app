...
export const handleListServiceByLoginUser = async (req, res) => {
	...
	new ApiResponse(
		res,
		200,
		"Successfully Fetched Service List by Logged in User",
		response
	);
};
...
export const handleListServiceByEmail = async (req, res) => {
	...
	if (!rows.length)
		new ApiResponse(
			res,
			200,
			"No Service Info Found With this Email..",
			response
		);
	else
		new ApiResponse(
			res,
			200,
			"Successfully Fetched Service List by Email",
			response
		);
};
...
export const handleGetServiceWithServiceId = async (req, res) => {
	...
	if (!rows[0])
		throw new NotFoundError(
			"No Service and Linked user Found With this Service Id.."
		);
...
export const handleDeleteServiceByServiceId = async (req, res) => {
	...
	if (rows.affectedRows <= 0)
		throw new NotImplemented("Something Went wrong in delete Service..");
